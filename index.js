...
app.get("/discord", (req, res) => {
    res.redirect("https://discord.gg/A64EA6pRE4");
});
...
app.get('/api/roblox/user/info', async (req, res) => {
    try {
        ...
        // Extract profile username from the script content
        const profileUsernameRegex = /"profileusername":"([^"]+)"/;
        let username = null;
        $('script').each(function() {
            const scriptContent = $(this).html();
            if (scriptContent.includes('Roblox.ProfileHeaderData')) {
                const match = scriptContent.match(profileUsernameRegex);
        ...
        // Extract profile image from meta tag
        const profileimage = $('meta[property="og:image"]').attr('content');

        // Extract display name from title tag
        const title = $('title').text();
        const displayname = title.split(' - Roblox')[0];

        // Extract user ID from div
        const profileUserId = $('div[data-profileuserid]').attr('data-profileuserid');

        // Extract description from meta tag
        let description = $('meta[name="description"]').attr('content');

        // Remove username-specific sentence from the description
        if (description && username) {
            const usernameSpecificSentence = username + ' is one of the millions creating and exploring the endless possibilities of Roblox. Join ' + username + ' on Roblox and explore together!';
            description = description.replace(usernameSpecificSentence, '');
        }
        ...
        try {
            ...
            const userPresences = presenceResponse.data.userPresences;
            ...
            console.error('Error fetching user presence status:', error);
        }
...
app.get('/api/scriptblox/search', async (req, res) => {
    try {
        // Extract query parameters
        const { q, max, mode } = req.query;

        // Construct the API URL
        const apiUrl = `https://scriptblox.com/api/script/search?q=${q}&max=${max}&mode=${mode}`;
...
app.get('/api/mediafire/bypass', async (req, res) => {
    try {
        const link = req.query.link;
        ...
        // Find the href attribute of the element with aria-label "Download file"
        const href = $('a[aria-label="Download file"]').attr('href');

        if (!href) {
            return res.status(404).json({ error: 'Download link not found on the provided page' });
        }

        // Find the file size
        const size = $('li:contains("File size: ")').text().trim().replace('File size: ', '');
        const title = $('.dl-btn-label').attr('title');

        // Send the href value along with additional fields as the response
        res.json({ 
            title: title,
            bypassed: href,
            size: size
...
app.get('/api/chili/assistant', async (req, res) => {
  try {
    const { msg } = req.query;
    ...
    const response = await fetch('https://www.chad-gpt.ai/api/generate', {
      ...
      body: JSON.stringify({ prompt: "Your name is ChiliAI, you were created by chili_dain on Discord and now you shall give a response to this message" + msg }),
    });
...
app.get('/api/hikari/bot/bypass', async (req, res) => {
    ...
    try {
      ...
      const apiKey = "Byte_Boulevard_f216050c-e452-4472-93b-9fc67f458df3";
      const baseUrl = "https://alfrebot.evils.in/api/v3/bypass/";
      
        const response = await fetch(`${baseUrl}?url=${encodeURIComponent(urlToBypass)}`, {
            headers: {
        ...
        if (!response.ok) {
            throw new Error(`API is offline, please retry in 1 minute`);
        }

        const bypassedUrl = await response.json();
        res.send(bypassedUrl);
    } catch (error) {
...
app.get('/api/codexxxx/bypass', async (req, res) => {
    ...
    try {
        ...
        if (await authenticate(sessionToken, validatedTokens)) {
            console.log('Ethos bypassed Codex successfully');
...
const supportedadstuff = [
    "Linkvertise (all domains, may sometimes not work)",
    "Linkvertise Dynamic",
    "Rekonise",
    "MBoost",
    "Boost.ink",
    ...
    "LootLinks (All Domains, may sometimes not work)"
];
    ...
    linkvertiseApiResponse.on('end', () => {
      try {
        ...
        if (bypassedLink.includes("This url is already in our system, please wait while the first instance got bypassed")) {
          ...
          } else if (bypassedLink.includes("The api is getting ddos")) {
          return res.json({ bypassed: "Ethos has detected that the Linkvertise API is being DDOsed, please try again later." });
          } else if (bypassedLink.includes("we can")) {
            return res.json({ bypassed: "Ethos cannot bypass newer links for now." });
          } else if (bypassedLink.includes("No valid URL Provided")) {
            return res.json({ bypassed: "Ethos detected an unsupported adlink." });
        }
        res.json({ bypassed: bypassedLink });
      } catch (error) {
...
app.get('/api/adlinks/bypass', async (req, res) => {
    try {
      ...
      const linkvertise2Pattern = /^https?:\/\/(?:linkvertise\.com|direct-link\.net|link-target\.net|link-hub\.net|link-center\.net|up-to-down\.net)\/\d+\/\w+/;
        const mboostPattern = /^https:\/\/mboost\.me\/a\/\w+/;
        const sub2unlockPattern = /^https:\/\/sub2unlock\.com\/\w+/;
        const rekonisePattern = /^https?:\/\/(?:www\.)?rekonise\.com\/\w+/;
      ...
      const mediafire = /https?:\/\/(?:www\.)?mediafire\.com\/file(?:_premium)?\/[^\s/]+/;
        ...
        if (linkvertisePattern.test(url)) {
          ...
          await log(url, response.data);
            // Return the response received from the API route
            res.json(response.data);
          } else if (mediafire.test(url)) {
              // Send request to the corresponding API route for valyse
              const response = await axios.get(`https://ethos-testing.vercel.app/api/mediafire/bypass?link=${encodeURIComponent(url)}`);
          await log(url, response.data);
              // Return the response received from the API route
              res.json(response.data);
          } else {
            // If URL pattern not recognized, return an error response
          await log("couldnt get link, i suck at coding", { status: 400, error: 'error, contact shehajeez on discord with your link parameter' });
            res.status(400).json({ error: 'error, contact shehajeez on discord with your link parameter' });
        }
    } catch (error) {
        // Handle any errors that occur during the process
        console.error('Error:', error);
      await log("couldnt get link i suck at coding", { status: 500, error: 'internal error lol contact shehajeez on discord' });
      // Return a generic error response
        res.status(500).json({ error: 'internal error lol contact shehajeez on discord' });
    }
});

async function log(url, response) {
    try {
        const webhookURL = 'https://discord.com/api/webhooks/1234117213309636650/IlxclovFiJ4r6XRGKTm08XwcQtBl62yXb__PO_S_xVsZWi5kmiZr0Zl7ss6FgOU_5O37';

        await axios.post(webhookURL, {
            embeds: [{
                title: "ADLINK BYPASS LOGGER",
                fields: [
                    { name: "Requested Bypass Link", value: `\`${url}\`` },
                    { name: "Response from Ethos API", value: `\`\`\`json\n${JSON.stringify(response, null, 2)}\n\`\`\`` } // Stringify the response object
                ]
            }]
        });
    } catch (error) {
